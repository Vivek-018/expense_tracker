...
// //         <span className="text-lg font-semibold text-emerald-400">Total: {totalDisplay}</span>
// //       </div>
...
//   // Derive categories from loaded expenses for the filter dropdown
//   const categories = ['all', ...Array.from(new Set(expenses.map(e => e.category)))];

//   // Total is always computed client-side from the current visible list
//   const totalPaise = expenses.reduce((sum, e) => sum + e.amountPaise, 0);
//   const totalDisplay = `₹${(totalPaise / 100).toFixed(2)}`;
...
export function ExpenseDashboard() {
  ...
  useEffect(() => {
    fetch('/api/expenses')
      .then(res => res.json())
      .then(json => {
        const cats = Array.from(new Set((json.data as ExpenseDTO[]).map(e => e.category)));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <CategoryFilter
            value={category}
            options={['all', ...allCategories]} // {/* ← always full list */}
            onChange={setCategory}
          />
